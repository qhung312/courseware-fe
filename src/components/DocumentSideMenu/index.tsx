...
const DocumentSideMenu: React.FC<DocumentSideMenuProps> = ({
  ...
  return (
    <>
      <div
        className={`z-20 fixed top-0 pt-[72px] md:pt-[110.6px] xl:pt-[149px] max-h-screen ${
          params?.subject ? 'translate-x-[-100%]' : ''
        } md:translate-x-0 w-full md:w-[264px] lg:w-[332px] xl:w-[400px] 3xl:w-[500px]  min-h-screen h-screen transition-all duration-300`}
      >
        <div className='bg-white flex items-center px-5 xl:px-7 pt-5 md:pt-[42px] lg:pt-14  h-full max-h-full overflow-y-scroll'>
          <div className='w-full h-full space-y-6'>
            ...
            <span>
              ...
              <p className='blockA md:hidden transition duration-300 text-[#252641]'>
                {description}
              </p>
            </span>
              ...
              {subjects.map((subj: any, index: number) => {
                ...
                return (
                  <button
                    ...
                    className={`mb-4 group flex-1 flex justify-between items-center px-6 py-5 ${
                      isActive
